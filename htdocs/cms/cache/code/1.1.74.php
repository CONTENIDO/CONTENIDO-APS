...
?><?php
 if (!class_exists('NewsletterJobCollection')) { echo mi18n("ERROR_CLASS"); } else { $oClientLang = new cApiClientLanguage(false, $client, $lang); $oClient = new cApiClient($client); $oRecipients = new NewsletterRecipientCollection(); $sMessage = " "; unset($recipient); $frontendURL = cRegistry::getFrontendUrl(); $aSettings = array( 'JoinSel' => $oClientLang->getProperty('newsletter', 'joinsel'), 'JoinMultiple' => $oClientLang->getProperty('newsletter', 'joinmultiple'), 'JoinGroups' => $oClientLang->getProperty('newsletter', 'joingroups'), 'JoinMessageType' => $oClientLang->getProperty('newsletter', 'joinmessagetype'), 'FrontendLink' => $oClient->getProperty('newsletter', 'frontendlink'), 'FrontendConfirm' => "$C160", 'FrontendDel' => "$C160", 'SenderEMail' => $oClient->getProperty('global', 'sender-email'), 'HandlerID' => $oClientLang->getProperty('newsletter', 'idcatart') ); $sTemplate = 'get.tpl'; if ($aSettings['JoinSel'] == '' || $aSettings['JoinGroups'] == '') { $aSettings['JoinSel'] = "Default"; } if ($aSettings['FrontendConfirm'] == '') { $aSettings['FrontendConfirm'] = "ActivateUser"; } if ($aSettings['FrontendDel'] == '') { $aSettings['FrontendDel'] = "DeleteUser"; } if ($_POST['action'] == "subscribe") { if (!isset($_POST['email']) || !$_POST['email']) { $sMessage = mi18n("SPECIFY_EMAIL"); } elseif (!isValidMail($_POST['email']) || strpos($_POST['email'], ",") != false || strpos($_POST['email'], ";") != false) { $sMessage = mi18n("SPECIFY_VALID_EMAIL"); } elseif ($oRecipients->emailExists($_POST['email'])) { $sMessage = mi18n("EMAIL_REGISTERED"); } elseif ($_POST['privacy'] != 1) { $sMessage = mi18n("ACCEPT_POLICY"); } else { $sEMail = preg_replace('/[\r\n]+/', '', stripslashes($_POST['email'])); $sName = stripslashes($_POST["emailname"]); switch ($aSettings['JoinMessageType']) { case "user": if ($_POST["selNewsletterType"] == 1) { $iMessageType = 1; } else { $iMessageType = 0; } break; case "html": $iMessageType = 1; break; default: $iMessageType = 0; } switch ($aSettings['JoinSel']) { case "Selected": $recipient = $oRecipients->create($sEMail, $sName, 0, $aSettings['JoinGroups'], $iMessageType); break; case "UserSelected": $iSelCount = count($_POST['selNewsletterGroup']); if ($iSelCount == 0) { $recipient = $oRecipients->create($sEMail, $sName, 0, "", $iMessageType); } else { if ($iSelCount > 1 && $aSettings['JoinMultiple'] != "enabled") { $sMessage = mi18n("SELECT_ONE_GROUP"); } else { $aGroups = explode(",", $aSettings['JoinGroups']); $bError = false; foreach ($_POST['selNewsletterGroup'] as $iIDGroup) { if (!is_numeric($iIDGroup) || !in_array($iIDGroup, $aGroups)) { $bError = true; break; } } if ($bError) { $sMessage = mi18n("ERROR_REQUEST"); } else { $recipient = $oRecipients->create($sEMail, $sName, 0, implode(",", $_POST['selNewsletterGroup'])); } } } break; default: $recipient = $oRecipients->create($sEMail, $sName, 0, "", $iMessageType); } if ($recipient) { $sBody = mi18n("TXTMAILSUBSCRIBE") . "\n" . $frontendURL . "front_content.php?changelang=" . $lang . "&idcatart=" . $aSettings['HandlerID'] . "&confirm=" . $recipient->get("hash") . "\n\n"; $mailer = new cMailer(); $from = array( $aSettings['SenderEMail'] => $aSettings['SenderEMail'] ); $recipients = $mailer->sendMail($from, $sEMail, mi18n("NEWSLETTER_CONFIRMATION"), $sBody); if ($recipients > 0) { $sMessage = mi18n("SUBCRIBED"); if ($aSettings['FrontendLink'] == "enabled") { $oFrontendUsers = new cApiFrontendUserCollection(); if (!$oFrontendUsers->userExists($sEMail)) { if ($frontenduser = $oFrontendUsers->create($sEMail)) { if ($aSettings['FrontendConfirm'] == "ActivateUser") { $sMessage .= mi18n("TXT_AFTER_CONFIRMATION"); } } else { $sMessage .= mi18n("TXT_PROBLEM_CREATING_ACCOUNT"); } } } } else { $sMessage = mi18n("TXT_SENDING_CONFIRMATION_MAIL"); } } else { $sMessage = mi18n("TXT_PROBLEM_SUBSCRIBING_EMAIL"); } } } elseif ($_POST['action'] == "delete") { if (!isset($_POST['email']) || !$_POST['email']) { $sMessage = mi18n("SPECIFY_EMAIL"); } elseif (!isValidMail($_POST['email']) || strpos($_POST['email'], ",") != false || strpos($_POST['email'], ";") != false) { $sMessage = mi18n("SPECIFY_VALID_EMAIL"); } elseif ($recipient = $oRecipients->emailExists($_POST['email'])) { $sBody = mi18n("TXTMAILDELETE") . "\n" . $frontendURL . "front_content.php?changelang=" . $lang . "&idcatart=" . $aSettings['HandlerID'] . "&unsubscribe=" . $recipient->get("hash") . "\n\n"; $mailer = new cMailer(); $from = array( $aSettings['SenderEMail'] => $aSettings['SenderEMail'] ); $recipients = $mailer->sendMail($from, $recipient->get('email'), mi18n("NEWSLETTER_CANCEL"), $sBody); if ($recipients > 0) { $sMessage = mi18n("TXT_SUBSCRIBER_EMAIL_SEND"); } else { $sMessage = mi18n("PROBLEM_SENDING_CANCELATION_EMAIL"); } } else { $sMessage = mi18n("EMAIL_NOT_FOUND"); } } elseif (strlen($_GET['confirm']) == 30 && isAlphanumeric($_GET['confirm'])) { $oRecipients->setWhere("idclient", $client); $oRecipients->setWhere("idlang", $lang); $oRecipients->setWhere("hash", $_GET['confirm']); $oRecipients->query(); if (($recipient = $oRecipients->next()) !== false) { $iID = $recipient->get("idnewsrcp"); $sEMail = $recipient->get("email"); $recipient->set("confirmed", 1); $recipient->set("confirmeddate", date("Y-m-d H:i:s"), false); $recipient->set("deactivated", 0); $recipient->store(); $sMessage = mi18n("CONFIRMED_SUBSCRIPTION_NEWSLETTER"); $oNewsletters = new NewsletterCollection(); $oNewsletters->setWhere("idclient", $client); $oNewsletters->setWhere("idlang", $lang); $oNewsletters->setWhere("welcome", '1'); $oNewsletters->query(); if (($oNewsletter = $oNewsletters->next()) !== false) { $aRecipients = array(); $oNewsletter->sendDirect($aSettings['HandlerID'], $iID, false, $aRecipients); $sMessage .= mi18n("WELCOME_NEWSLETTER"); } if ($aSettings['FrontendLink'] == "enabled" && $aSettings['FrontendConfirm'] == "ActivateUser") { $oFrontendUsers = new cApiFrontendUserCollection(); $oFrontendUsers->setWhere("idclient", $client); $oFrontendUsers->setWhere("username", $sEMail); $oFrontendUsers->query(); if (($frontenduser = $oFrontendUsers->next()) !== false) { $frontenduser->set("active", 1); $sPassword = substr(md5(rand()), 0, 8); $frontenduser->set("password", $sPassword); $frontenduser->store(); $sMessage .= mi18n("TXT_ACCOUNT_ACTIVATED"); $sMessage .= mi18n("USERNAME_COLON") . $sEMail . mi18n("

_BR ") . $sPassword; $sBody = mi18n("TXTMAILPASSWORD") . "\n\n" . mi18n("USERNAME_COLON") . $sEMail . "\n" . mi18n("PASSWORD_COLON ") . $sPassword . "\n\n" . mi18n("LOGIN_CLICK") . $frontendURL . "front_content.php?changelang=" . $lang; $mailer = new cMailer(); $from = array( $aSettings['SenderEMail'] => $aSettings['SenderEMail'] ); $recipients = $mailer->sendMail($from, $sEMail, mi18n("WEBSITE_ACCOUNT"), $sBody); if ($recipients > 0) { $sMessage .= mi18n("ACOUNT_DETAILS_EMAIL"); } else { $sMessage .= mi18n("TXT_PROBLEM_ACCOUNTDETAILS"); } } else { $sMessage .= mi18n("PROBLEM_ACTIVATING_EMAIL_ACCOUNT"); } } } else { $sMessage = mi18n("PROBLEM_CONFIRMING_SUBSCRIPTION"); } } elseif (strlen($_GET['stop']) == 30 && isAlphanumeric($_GET['stop'])) { $oRecipients->setWhere("idclient", $client); $oRecipients->setWhere("idlang", $lang); $oRecipients->setWhere("hash", $_GET['stop']); $oRecipients->query(); if (($recipient = $oRecipients->next()) !== false) { $recipient->set("deactivated", 1); $recipient->store(); $sMessage = mi18n("NEWSLETTER_SUBSCRIPTION_PAUSED"); } else { $sMessage = mi18n("PROBLEM_PAUSING_NEWSLETTER_SUBSCRIPTION"); } } elseif (strlen($_GET['goon']) == 30 && isAlphanumeric($_GET['goon'])) { $oRecipients->setWhere("idclient", $client); $oRecipients->setWhere("idlang", $lang); $oRecipients->setWhere("hash", $_GET['goon']); $oRecipients->query(); if (($recipient = $oRecipients->next()) !== false) { $recipient->set("deactivated", 0); $recipient->store(); $sMessage = mi18n("NEWSLETTER_SUBSCRIPTION_RESUMED"); } else { $sMessage = mi18n("PROBLEM_RESUMING_NEWSLETTER_SUBSCRIPTION"); } } elseif (strlen($_GET['unsubscribe']) == 30 && isAlphanumeric($_GET['unsubscribe'])) { $oRecipients->setWhere("idclient", $client); $oRecipients->setWhere("idlang", $lang); $oRecipients->setWhere("hash", $_GET['unsubscribe']); $oRecipients->query(); if (($recipient = $oRecipients->next()) !== false) { $sEMail = $recipient->get("email"); $oRecipients->delete($recipient->get("idnewsrcp")); $sMessage = mi18n("EMAIL_ADDRESS_REMOVED"); if ($aSettings['FrontendLink'] == "enabled") { $oFrontendUsers = new cApiFrontendUserCollection(); $oFrontendUsers->setWhere("idclient", $client); $oFrontendUsers->setWhere("username", $sEMail); $oFrontendUsers->query(); if (($frontenduser = $oFrontendUsers->next()) !== false) { switch ($aSettings['FrontendDel']) { case "DeleteUser": $oFrontendUsers->delete($frontenduser->get("idfrontenduser")); $sMessage .= mi18n("WEBSITE_ACCOUNT_DELETED"); break; case "DisableUser": $frontenduser->set("active", 0); $frontenduser->store(); $sMessage .= mi18n("WEBSITE_ACCOUNT_DISABLED"); break; default: } } } } else { $sMessage = mi18n("PROBLEM_REMOVING_EMAIL_ADDRESS"); } } $tpl = cSmartyFrontend::getInstance(); $tpl->assign('CONTENT', $sMessage); $tpl->display($sTemplate); } ?>
                <?php
$cCurrentModule = 34;
$cCurrentContainer = 170;
?><?php
 if (class_exists('NewsletterJobCollection')) { $oClientLang = new cApiClientLanguage(false, cRegistry::getClientId(), cRegistry::getLanguageId()); $aSettings = array( 'JoinSel' => $oClientLang->getProperty('newsletter', 'joinsel'), 'JoinMultiple' => $oClientLang->getProperty('newsletter', 'joinmultiple'), 'JoinGroups' => $oClientLang->getProperty('newsletter', 'joingroups'), 'JoinMessageType' => $oClientLang->getProperty('newsletter', 'joinmessagetype'), 'OptNewWindow' => "$C170" ); $sTemplate = 'get.tpl'; $cmsLinkeditor = "<a id=\"m15\" class=\"link_list\" alt=\"die Datenschutzbestimmungen\" title=\"die Datenschutzbestimmungen\" target=\"_blank\" href=\"http://conclaus-git.local/cms/front_content.php?idart=8\">die Datenschutzbestimmungen</a>"; if ($aSettings["JoinSel"] == "" || ($aSettings["JoinSel"] == "UserSelected" && $aSettings["JoinGroups"] == "")) { $aSettings["JoinSel"] = "Default"; } $tpl = cSmartyFrontend::getInstance(); $tpl->assign('FORM_ACTION', 'front_content.php?changelang=' . cRegistry::getLanguageId() . '&idcatart=' . $oClientLang->getProperty('newsletter', 'idcatart')); unset($oClientLang); if ($aSettings["OptNewWindow"]) { $tpl->assign('FORM_TARGET', ' target="_blank"'); } else { $tpl->assign('FORM_TARGET', ''); } $tpl->assign('EMAILNAME', mi18n("NAME")); $tpl->assign('EMAIL', mi18n("E_MAIL")); $sTmpHTML = ""; if ($aSettings["JoinSel"] == "UserSelected") { $oRcpGroups = new NewsletterRecipientGroupCollection(); $oRcpGroups->setWhere('idclient', cRegistry::getClientId()); $oRcpGroups->setWhere('idlang', cRegistry::getLanguageId()); $oRcpGroups->setWhere('defaultgroup', '0'); $oRcpGroups->setWhere('idnewsgroup', explode(',', $aSettings["JoinGroups"]), 'IN'); $oRcpGroups->setOrder('groupname ASC'); $oRcpGroups->query(); if ($oRcpGroups->Count() > 0) { $oLblGroupSel = new cHTMLLabel(mi18n("SELECT"), "selNewsletterGroup"); $oSelGroup = new cHTMLSelectElement("selNewsletterGroup[]", "", "selNewsletterGroup"); $oSelGroup->setSize(2); $oSelGroup->setClass(""); if ($aSettings["JoinMultiple"] == "enabled") { $oSelGroup->setMultiselect(); } while (false !== $oRcpGroup = $oRcpGroups->next()) { $iID = $oRcpGroup->get("idnewsgroup"); $oOption = new cHTMLOptionElement($oRcpGroup->get("groupname"), $iID); $oSelGroup->addOptionElement($iID, $oOption); } $sTmpHTML .= '         ' . $oLblGroupSel->toHTML() . "\n"; $sTmpHTML .= '         ' . $oSelGroup->render() . "\n"; $sTmpHTML .= '         <br class="y"/>'; } } if ($aSettings['JoinMessageType'] == 'user') { $oLblType = new cHTMLLabel(mi18n("TYPE"), "selNewsletterType"); $oSelType = new cHTMLSelectElement("selNewsletterType", "", "selNewsletterType"); $oSelType->setSize(1); $oSelType->setClass(""); $oOption = new cHTMLOptionElement(mi18n("TEXT_ONLY"), 0); $oSelType->addOptionElement(0, $oOption); $oOption = new cHTMLOptionElement(mi18n("HTML"), 1); $oSelType->addOptionElement(1, $oOption); $sTmpHTML .= '         ' . $oSelType->render() . "\n"; $sTmpHTML .= '         <br class="y"/>'; } $tpl->assign('EXTRAHTML', $sTmpHTML); $tpl->assign('NEWSLETTER', mi18n("NEWSLETTER_SRC")); $tpl->assign('SUBSCRIBE', mi18n("SUBSCRIBE_SRC")); $tpl->assign('DELETE', mi18n("UNSUBSCRIBE")); $tpl->assign("ABSCHICKEN", mi18n("SUBMIT")); $tpl->assign("LOESCHEN", mi18n("DELETE")); $tpl->assign('JOIN', mi18n("JOIN")); $tpl->assign('LINKEDITOR', $cmsLinkeditor); $tpl->assign('PRIVACY_TEXT_PART1', mi18n("READ_AND_ACCEPT_1")); $tpl->assign('PRIVACY_TEXT_PART2', mi18n("READ_AND_ACCEPT_2 ")); $tpl->display($sTemplate); } ?>
                <?php
$cCurrentModule = 0;
...
?><?php
 if (!$contenido) { $session = cRegistry::getSession(); if (array_key_exists('acceptCookie', $_GET)) { $allowCookie = $_GET['acceptCookie'] === '1'? 1 : 0; setcookie('allowCookie', $allowCookie); $session->register('allowCookie'); } elseif (array_key_exists('allowCookie', $_COOKIE)) { $allowCookie = $_COOKIE['allowCookie'] === '1'? 1 : 0; $session->register('allowCookie'); } if (!isset($allowCookie)) { $tpl = cSmartyFrontend::getInstance(); $tpl->assign('trans', array( 'title' => mi18n("TITLE"), 'infoText' => mi18n("INFOTEXT"), 'userInput' => mi18n("USERINPUT"), 'accept' => mi18n("ACCEPT"), 'decline' => mi18n("DECLINE") )); function script_cookie_directive_add_get_params($uri) { foreach($_GET as $getKey => $getValue) { if (strpos($uri, '?' . $getKey . '=') !== false || strpos($uri, '&' . $getKey . '=') !== false) { continue; } if (strpos($uri, '?') === false) { $uri .= '?'; } else { $uri .= '&'; } $uri .= htmlentities($getKey) . '=' . htmlentities($getValue); } return $uri; } $acceptUrl = script_cookie_directive_add_get_params(cUri::getInstance()->build(array( 'idart' => cRegistry::getArticleId(), 'lang' => cRegistry::getLanguageId(), 'acceptCookie' => 1 ), true)); $tpl->assign('pageUrlAccept', $acceptUrl); $denyUrl = script_cookie_directive_add_get_params(cUri::getInstance()->build(array( 'idart' => cRegistry::getArticleId(), 'lang' => cRegistry::getLanguageId(), 'acceptCookie' => 0 ), true)); $tpl->assign('pageUrlDeny', $denyUrl); $tpl->display('get.tpl'); } } ?>
    ...
    <script src="//conclaus-git.local/cms/cache/newsletter_subscription.js" type="text/javascript"></script></body>
