...
<meta name="keywords" content="contenido, content, newsletter, extensive, oxid2con, online, management, interface, scheduled, portal, options, enables, administration, implementation, manager" />
</head>
...
?><?php
 defined('CON_FRAMEWORK') || die('Illegal call: Missing framework initialization - request aborted.'); $header = "<p>Plugins</p>"; if (cRegistry::isBackendEditMode()) { $label = mi18n("LABEL_HEADER_FIRST"); } else { $label = NULL; $header = str_replace('&nbsp;', ' ', $header); $header = strip_tags($header); $header = trim($header); } $tpl = cSmartyFrontend::getInstance(); $tpl->assign('label', $label); $tpl->assign('header', $header); $tpl->display('get.tpl'); ?>
                <?php
$cCurrentModule = 2;
$cCurrentContainer = 120;
?><?php
 defined('CON_FRAMEWORK') || die('Illegal call: Missing framework initialization - request aborted.'); $header = "<p><span><span class=\"hps\">Plugins</span> <span class=\"hps\">are</span> <span class=\"hps\">additional functions that connect </span><span class=\"hps\">to</span> <span class=\"hps\">the core</span> <span class=\"hps\">of</span> <span class=\"hps\">CONTENIDO</span><span>.</span></span></p>"; if (cRegistry::isBackendEditMode()) { $label = mi18n("LABEL_HEADER_SECOND"); } else { $label = NULL; $header = str_replace('&nbsp;', ' ', $header); $header = strip_tags($header); $header = trim($header); } $tpl = cSmartyFrontend::getInstance(); $tpl->assign('label', $label); $tpl->assign('header', $header); $tpl->display('get.tpl'); ?>
                <?php
$cCurrentModule = 5;
...
?><?php
 defined('CON_FRAMEWORK') || die('Illegal call: Missing framework initialization - request aborted.'); $text = "<p><span><span class=\"hps\">The following</span> <span class=\"hps\">plugins are included</span> <span class=\"hps\">in the standard version</span> <span class=\"hps\">of CONTENIDO</span><span>:</span></span></p>
<p></p>
<ul>
<li>
<p><strong>Newsletter</strong><strong></strong>: The newsletter module integrates newsletter publication into your online services by providing opt-in and opts-out functionality, including extensive user management. Implementation of HTML and plain text newsletters has never been easier in CONTENIDO.</p>
</li>
</ul>
<p></p>
<ul>
<li>
<p><strong>Contact and Form Manager</strong><strong></strong>: The form manager of CONTENIDO enables you to build forms within seconds. Content validation checks on input can be performed with regular expression and mail content can be enriched with form variables. CONTENIDO saves user generated content to its database and offers several export options.</p>
</li>
</ul>
<p></p>
<ul>
<li>
<p><strong>SEO Optimization</strong><strong></strong>: CONTENIDO enables SEO through search engine optimized URLs with ModRewrite, an automatic generation of meta-tags and page titles and additionally Google sitemaps.<strong></strong></p>
</li>
</ul>
<p></p>
<ul>
<li>
<p><strong></strong><strong>OXID2CON Portal</strong><strong></strong>: OXID2CON Portal creates the perfect interface between an online shop and a content management system thus OXID e-commerce software and the CMS CONTENIDO pairing perfectly. OXID2CON Portal increases the attraction of your online shop by adding extensive modules and elements for the integration and presentation of content. The interface delivers valuable content to enrich your shop with extensive product information, teasers, picture galleries and blogs creating an exciting online shopping experience.</p>
</li>
</ul>
<ul>
<li>
<p>&nbsp;<strong>Scheduled Tasks</strong>: The administration module of CONTENIDO provides options for managing and automatically executing tasks on a scheduled basis (cronjobs).</p>
</li>
</ul>"; if (cRegistry::isBackendEditMode()) { $label = mi18n("LABEL_TEXT"); } else { $label = NULL; } $tpl = cSmartyFrontend::getInstance(); $tpl->assign('label', $label); $tpl->assign('text', $text); $tpl->display('get.tpl'); ?>
                </div>
                <?php
$cCurrentModule = 6;
...
?><?php
 defined('CON_FRAMEWORK') || die('Illegal call: Missing framework initialization - request aborted.'); $tpl = cSmartyFrontend::getInstance(); $tpl->assign('isBackendEditMode', cRegistry::isBackendEditMode()); $tpl->assign('label', mi18n("LABEL_TEASERIMAGE")); $tpl->assign('image', ""); $tpl->assign('editor', "<img id=\"m15\" src=\"\" alt=\"\" title=\"\" />"); $tpl->display('get.tpl'); ?>
                <?php
$cCurrentModule = 0;
