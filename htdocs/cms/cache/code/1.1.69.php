...
<meta name="description" content="CONTENIDO Upgrade 4.9" />
<meta name="keywords" content="contenido, business, gewährleisten, aktualisieren, spezialisten, reibungslose, bestehendes, umstellung, info@4fbde, preiswerte, verfügen, erstellen" />
</head>
...
?><?php
 defined('CON_FRAMEWORK') || die('Illegal call: Missing framework initialization - request aborted.'); $header = "<p>CONTENIDO Upgrade 4.9</p>"; if (cRegistry::isBackendEditMode()) { $label = mi18n("LABEL_HEADER_FIRST"); } else { $label = NULL; $header = str_replace('&nbsp;', ' ', $header); $header = strip_tags($header); $header = trim($header); } $tpl = cSmartyFrontend::getInstance(); $tpl->assign('label', $label); $tpl->assign('header', $header); $tpl->display('get.tpl'); ?>
                <?php
$cCurrentModule = 2;
$cCurrentContainer = 120;
?><?php
 defined('CON_FRAMEWORK') || die('Illegal call: Missing framework initialization - request aborted.'); $header = "<div>
<div>
<div>
<p>Mehr Sicherheit und Performance!</p>
</div>
</div>
</div>"; if (cRegistry::isBackendEditMode()) { $label = mi18n("LABEL_HEADER_SECOND"); } else { $label = NULL; $header = str_replace('&nbsp;', ' ', $header); $header = strip_tags($header); $header = trim($header); } $tpl = cSmartyFrontend::getInstance(); $tpl->assign('label', $label); $tpl->assign('header', $header); $tpl->display('get.tpl'); ?>
                <?php
$cCurrentModule = 5;
...
?><?php
 defined('CON_FRAMEWORK') || die('Illegal call: Missing framework initialization - request aborted.'); $text = "<div>
<div>
<div>
<p>Lassen Sie ihr bestehendes System aktualisieren. Die Spezialisten der four for business AG verf&uuml;gen &uuml;ber 13 Jahre CONTENIDO Know-how. Wir gew&auml;hrleisten eine reibungslose und preiswerte Umstellung Ihres Systems. Lassen Sie sich ein Angebot erstellen - es lohnt sich.</p>
<p>Sprechen Sie uns an!</p>
<p>four for business AG</p>
<p> 49 (0) 69 801082-0 | info@4fb.de | www.4fb.de</p>
</div>
</div>
</div>"; if (cRegistry::isBackendEditMode()) { $label = mi18n("LABEL_TEXT"); } else { $label = NULL; } $tpl = cSmartyFrontend::getInstance(); $tpl->assign('label', $label); $tpl->assign('text', $text); $tpl->display('get.tpl'); ?>
                </div>
                <?php
$cCurrentModule = 6;
