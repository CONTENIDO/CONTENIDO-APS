...
<meta name="description" content="Downloadliste" />
<meta name="keywords" content="contenido, produktdatenblätter, verzeichnisses, informationen, ausgewählten, dateigröße, bildmaterial, preislisten, verfügung, darstellt, besuchern, inklusive" />
</head>
...
?><?php
 defined('CON_FRAMEWORK') || die('Illegal call: Missing framework initialization - request aborted.'); $text = "<p>Haben Sie Preislisten, Produktdatenbl&auml;tter, Bildmaterial oder andere Informationen, die Sie den Besuchern Ihrer Website zum Download zur Verf&uuml;gung stellen wollen? CONTENIDO liefert Ihnen auch daf&uuml;r ein fertiges Modul, das alle Dateien eines ausgew&auml;hlten Verzeichnisses inklusive Datentyp und Dateigr&ouml;&szlig;e ausliest und in Form einer Liste darstellt.</p>"; if (cRegistry::isBackendEditMode()) { $label = mi18n("LABEL_TEXT"); } else { $label = NULL; } $tpl = cSmartyFrontend::getInstance(); $tpl->assign('label', $label); $tpl->assign('text', $text); $tpl->display('get.tpl'); ?>
                </div>
                <?php
$cCurrentModule = 6;
...
?><?php
 defined('CON_FRAMEWORK') || die('Illegal call: Missing framework initialization - request aborted.'); if (cRegistry::isBackendEditMode()) { $label = mi18n("LABEL_HEADER_DOWNLOADLIST"); } else { $label = NULL; } ob_start(); echo "";?><?php
                    $fileList = new cContentTypeFilelist('<?xml version="1.0" encoding="utf-8"?>
<filelist><title></title><style>cms_filelist_style_default.html</style><directories><array_value>downloads</array_value></directories><incl_subdirectories>false</incl_subdirectories><manual>false</manual><sort>filename</sort><incl_metadata>true</incl_metadata><extensions></extensions><sortorder>asc</sortorder><filesizefilter_from>0</filesizefilter_from><filesizefilter_to>0</filesizefilter_to><ignore_extensions>true</ignore_extensions><manual_files></manual_files><filecount>0</filecount><md_description_limit>0</md_description_limit><md_medianame_limit>0</md_medianame_limit><md_copyright_limit>0</md_copyright_limit><md_keywords_limit>0</md_keywords_limit><md_internal_notice_limit>0</md_internal_notice_limit><creationdatefilter_from>0</creationdatefilter_from><creationdatefilter_to>0</creationdatefilter_to><modifydatefilter_from>0</modifydatefilter_from><modifydatefilter_to>0</modifydatefilter_to></filelist>
', 1, array());
                    echo $fileList->generateFileListCode();
                 ?><?php echo ""; $filelist = ob_get_contents(); ob_end_clean(); $tpl = cSmartyFrontend::getInstance(); $tpl->assign('label', $label); $tpl->assign('filelist', $filelist); $tpl->display('get.tpl'); ?>
                <?php
$cCurrentModule = 0;
...
?><?php
 if (!$contenido) { $session = cRegistry::getSession(); if (array_key_exists('acceptCookie', $_GET)) { $allowCookie = $_GET['acceptCookie'] === '1'? 1 : 0; setcookie('allowCookie', $allowCookie); $session->register('allowCookie'); } elseif (array_key_exists('allowCookie', $_COOKIE)) { $allowCookie = $_COOKIE['allowCookie'] === '1'? 1 : 0; $session->register('allowCookie'); } if (!isset($allowCookie)) { $tpl = cSmartyFrontend::getInstance(); $tpl->assign('trans', array( 'title' => mi18n("TITLE"), 'infoText' => mi18n("INFOTEXT"), 'userInput' => mi18n("USERINPUT"), 'accept' => mi18n("ACCEPT"), 'decline' => mi18n("DECLINE") )); function script_cookie_directive_add_get_params($uri) { foreach($_GET as $getKey => $getValue) { if (strpos($uri, '?' . $getKey . '=') !== false || strpos($uri, '&' . $getKey . '=') !== false || strpos($uri, '&amp;' . $getKey . '=') !== false) { continue; } if (strpos($uri, '?') === false) { $uri .= '?'; } else { $uri .= '&amp;'; } $uri .= htmlentities($getKey) . '=' . htmlentities($getValue); } return $uri; } $acceptUrl = script_cookie_directive_add_get_params(cUri::getInstance()->build(array( 'idart' => cRegistry::getArticleId(), 'lang' => cRegistry::getLanguageId(), 'acceptCookie' => 1 ), true)); $tpl->assign('pageUrlAccept', $acceptUrl); $denyUrl = script_cookie_directive_add_get_params(cUri::getInstance()->build(array( 'idart' => cRegistry::getArticleId(), 'lang' => cRegistry::getLanguageId(), 'acceptCookie' => 0 ), true)); $tpl->assign('pageUrlDeny', $denyUrl); $tpl->display('get.tpl'); } } ?>
    ...
    <script src="//conclaus-git.local/cms/cache/content_download_list.js" type="text/javascript"></script></body>
